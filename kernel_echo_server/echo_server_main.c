...
{
    static struct socket *in_socket;
...
static int __init echo_server_init(void)
{
    ...
    echo_server_thread = kthread_create(echo_server, NULL, "echod");
	if (IS_ERR(echo_server_thread)) {
        printk(KERN_ERR "echo server: unable to create thread\n");
		return PTR_ERR(echo_server_thread);
    }

	ret = sock_create_kern(AF_INET, SOCK_STREAM, 0, &echo_server_socket);
	if (ret < 0) {
    ...
    printk(KERN_INFO "echo server: listening for connections on ip %s, port %d\n",
            echo_server_ip, echo_server_port);
